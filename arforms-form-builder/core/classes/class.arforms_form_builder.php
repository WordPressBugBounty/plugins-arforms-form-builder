...
class arforms_form_builder{
    ...
    function __construct(){
		...
		if( !empty($arforms_migration_flag) ){
			...
			if ( $blog_id && IS_WPMU ) {
				...
				$tbl_arf_paypal_forms = "{$prefix}arf_paypal_forms";
				$tbl_arf_paypal_order = "{$prefix}arf_paypal_order";
				...
				$tbl_arf_paypal_forms = $wpdb->prefix. "arf_paypal_forms";
				$tbl_arf_paypal_order = $wpdb->prefix. "arf_paypal_order";
			}
        ...
        add_action( 'admin_enqueue_scripts', array( $this, 'arforms_load_assets'), 10 );
		...
		if( true == $force || version_compare( $this->arforms_get_premium_version(), '6.1', '<' ) ){
			...
			if( !empty( $all_lite_forms ) ){
				...
				foreach( $all_lite_forms as $lite_frm ){
					...
					$arforms_lite_form_new_ids[ $lite_form_id ] = $migrated_form_id;
					...
					if( !empty( $updated_field_order ) ){
						...
						$lite_form_opts['arf_field_order'] = wp_json_encode( $updated_field_order );
					...
					if( !empty( $arforms_lite_form_new_ids ) ){
						global $tbl_arf_paypal_forms, $tbl_arf_paypal_order;
						foreach( $arforms_lite_form_new_ids as $old_form_id => $new_form_id ){
							$fetch_paypal_forms = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM $tbl_arf_paypal_forms WHERE form_id = %d", $old_form_id ) );

							if( !empty( $fetch_paypal_forms ) ){
								$fid = $fetch_paypal_forms->id;
								$fname = $fetch_paypal_forms->form_name;
								$fopt = $fetch_paypal_forms->options;
								$created_at = $fetch_paypal_forms->created_at;

								$wpdb->insert(
									$tbl_arf_paypal_forms,
									array(
										'form_id' => $new_form_id,
										'form_name' => $fname,
										'options' => $fopt,
							...
							$fetch_paypal_form_orders = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM $tbl_arf_paypal_order WHERE form_id = %d", $old_form_id ) );

							if( !empty( $fetch_paypal_form_orders ) ){
								foreach( $fetch_paypal_form_orders as $order_data ){
									...
									$payment_date = $order_data->payment_date;
									$is_verified = $order_data->is_verified;
    ...
    function arforms_load_assets(){
		...
		if ( isset( $_REQUEST['page'] ) && $_REQUEST['page'] != '' && sanitize_text_field( $_REQUEST['page'] ) == 'ARForms' && ( isset( $_REQUEST['arfaction']) && ( sanitize_text_field( $_REQUEST['arfaction'] ) == 'edit' || sanitize_text_field( $_REQUEST['arfaction'] ) == 'new' || sanitize_text_field( $_REQUEST['arfaction'] ) == 'duplicate' || sanitize_text_field( $_REQUEST['arfaction'] ) == 'update' )) ) {

			/* nouislider js css*/
			wp_register_script( 'nouislider', ARFLITEURL . '/js/nouislider.js', array( 'jquery' ), $this->arforms_get_assets_version() );
			wp_enqueue_script( 'nouislider');

			wp_register_style( 'nouislider', ARFLITEURL . '/css/nouislider.css', array(), $this->arforms_get_assets_version() );
			wp_enqueue_style( 'nouislider' );

			/* selectpicker js/css */
			wp_enqueue_script( 'arf_selectpicker', ARFLITEURL . '/js/arf_selectpicker.js', array('jquery'), $this->arforms_get_assets_version() );

			/* phone utils js */
			wp_enqueue_script( 'arformslite_editor_phone_utils', ARFLITEURL . '/js/arf_phone_utils.js', array(), $this->arforms_get_assets_version() );
			wp_enqueue_script( 'arformslite_editor_phone_intl_input', ARFLITEURL . '/js/intlTelInput.min.js', array(), $this->arforms_get_assets_version() );

			wp_enqueue_script( 'bootstrap' );
			wp_register_style('arfbootstrap-datepicker-css', ARFLITEURL . '/bootstrap/css/bootstrap-datetimepicker.css', array(), $this->arforms_get_assets_version() );
            wp_enqueue_style('arfbootstrap-datepicker-css');

			wp_enqueue_script('arf_js_color',ARFLITEURL.'/js/jscolor.js',array(), $this->arforms_get_assets_version());
		...
		if( !empty( $_GET['page'] ) && 'ARForms-status' == $_GET['page'] ){
			...
			wp_enqueue_style('arf_selectpicker', ARFLITEURL . '/css/arf_selectpicker.css', array(), $this->arforms_get_assets_version());
			wp_enqueue_script('arf_selectpicker', ARFLITEURL . '/js/arf_selectpicker.js', array(), $this->arforms_get_assets_version());
		}
		
		if( !empty( $_GET['page'] ) && 'ARForms' == $_GET['page'] && !empty( $_GET['arfaction'] ) ){
		...
		if( !empty( $_GET['page'] ) && 'ARForms-settings' == $_GET['page'] ){
			...
			wp_register_script( 'bootstrap', ARFLITEURL . '/bootstrap/js/bootstrap.min.js', array( 'jquery' ), $this->arforms_get_assets_version() );
			wp_enqueue_script( 'bootstrap' );
		...
		if( !empty( $_GET['page'] ) && "ARForms-popups" == $_REQUEST['page'] ){
		...
		if (isset($_REQUEST['page']) && $_REQUEST['page'] != '' && $_REQUEST['page'] == "AROrder-entries") {
		...
		if( !empty( $_GET['page'] ) && 'ARForms-addons' == $_GET['page'] ){

			wp_register_style( 'arf_addon_list', ARFLITEURL . '/css/arf_addon.css', array(), $this->arforms_get_assets_version() );
			wp_enqueue_style( 'arf_addon_list' );
    ...
    public static function arfliteupgrade( $old_db_version = false ) {
		...
		global $wpdb, $arflitedbversion, $tbl_arf_fields, $tbl_arf_forms, $tbl_arf_entries, $tbl_arf_entry_values, $tbl_arf_debug_log_setting, $tbl_arf_settings,$arf_paypal_version, $tbl_arf_paypal_forms, $tbl_arf_paypal_order;
		...
		if ( $arflitedbversion != $old_db_version ) {
			...
			$table_name = $tbl_arf_paypal_order;
			...
			$table_name = $tbl_arf_paypal_forms;
			...
			$wpdb->insert(
				$tbl_arf_settings,
				array(
					'setting_name' => 'arforms_gcaptcha',
					'setting_value' => 0,
					'setting_type' => 'arforms_module'
				),
				array(
					'%s',
					'%s',
    ...
    public static function arflite_default_options() {
		return array(
			...
			'empty_msg'                     => 'Sorry, you cannot submit an empty form.',
			'submit_value'                  => 'Submit',
			...
			'arfmainformiplogging'          => 0,
			'arf_load_js_css'               => wp_json_encode( array() ),
			'anonymous_data'              	=> 0,
			'tc_pubkey'              		=> '',
			'tc_privkey'              		=> '',
    ...
    function arforms_register_menu(){
		...
		global $arformsmain,$tbl_arf_entries,$wpdb, $tbl_arf_forms;
		...
		$unread_counter = '<span class ="arf_entries_flag"> '.$unread_entries_value.' </span>';

		if($unread_entries_value > 99){
			$unread_counter = '<span class ="arf_entries_flag"> 99+ </span>'; 
		}

		if($unread_entries_value == 0){
			$unread_counter = ''; 
		}
		...
		add_submenu_page( 'ARForms', 'ARForms' . ' | ' . __( 'Form Entries', 'arforms-form-builder' ), __( 'Form Entries', 'arforms-form-builder' ) .' '.$unread_counter, 'arfviewentries', 'ARForms-entries', array( $this, 'arforms_router' ) );
