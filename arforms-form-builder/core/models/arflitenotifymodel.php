...
class arflitenotifymodel {
	...
	function __construct() {
		...
		add_action('wp_mail_failed', array( $this, 'arformslite_wp_mail_failed_response') );

        add_action('wp_mail_succeeded', array( $this, 'arformslite_wp_mail_success_response'));

		add_action('arfliteaftercreateentry', array($this, 'arflite_prevent_paypal_to_stop_sending_email'),1,2);
		add_action('arfaftercreateentry', array($this, 'arflite_prevent_paypal_to_stop_sending_email'),1,2);
	}

	function arflite_prevent_paypal_to_stop_sending_email( $entry_id,$form_id ){

		if(empty($entry_id) || is_plugin_active( 'arformspaypal/arformspaypal.php' ) ) {
            return;
        }
        global $wpdb,$arfsettings,$arformsmain;
        ...
        $prevent_sending_email = apply_filters('arflite_prevent_paypal_to_stop_sending_email_outside',$prevent_sending_email,$entry_id,$form_id);

		$arf_is_paypal_enable = $arformsmain->arforms_get_settings('arforms_paypal','arforms_module');

		if( $arf_is_paypal_enable == 1 ){
			global $arf_paypal;
            remove_action('check_arflite_payment_gateway',array($arf_paypal,'arf_paypal_check_response'),20);
		}
	...
	function arformslite_wp_mail_success_response( $mesage ){
        ...
        if( true == $this->arforms_wpmail_sending ){
            ...
            $arf_error_msg=array(	
                'success' => 'true',
                'msg'     =>   wp_json_encode( $mesage ),
				'backtrace' => $backtrace
            );
            do_action('arforms_debug_log_entry','email_notification', 'WP Mail sent response', 'arforms_email_notiifcation', $arf_error_msg, $arf_debug_log_id);
        }
    }
	...
	function arflite_send_notification_email_user( $to_email, $subject, $message, $reply_to = '', $reply_to_name = '', $plain_text = true, $attachments = array(), $return_value = false, $use_only_smtp_settings = false, $check = false, $enable_debug = false, $user_nreplyto = '', $cc_email = '', $bcc_email = '' ) {
		...
		if ( isset( $smtp_server ) && $smtp_server == 'custom' ) {
			...
			if ( ! $mail->send() ) {
				...
				if ( $enable_debug ) {
					$smtp_debug_log = var_export( $mail, true );
				}
				...
				if ( ! empty( $use_only_smtp_settings ) ) {
					...
					echo wp_json_encode(
						array(
							'success' => 'false',
							'msg'     => esc_html__('Sorry! Email was not sent successfully.','arforms-form-builder') . '<a href="#arf_smtp_error" data-toggle="arfmodal">' . __( 'Check Full Log', 'arforms-form-builder' ) . '</a>',
			...
			if ( $mail->send() ) {
				...
				$arf_success_msg=array(	
					'success' => 'true',
					'msg'     =>   esc_html__('Email Sent Successfully', 'arforms-form-builder'),
					'send_to'	=> $to_email,
					'reply_to' => $reply_to,
					'reply_to_name' => $reply_to_name,	
				);
				do_action('arforms_debug_log_entry','email_notification', 'PHPMailer sent response', 'arforms_email_notiifcation', $arf_success_msg, $arf_debug_log_id);
				...
				do_action('arforms_debug_log_entry','email_notification', 'PHPMailer sent response', 'arforms_email_notiifcation', $arf_error_msg, $arf_debug_log_id);
			}
            ...
            if( $client->isAccessTokenExpired() ){
                ...
                if( !empty( $is_refreshed['error'] ) ){
					...
					return wp_json_encode(array('success' => 'false', 'msg' => $refreshed_token_err));
                }
				...
				$arformsmain->arforms_update_settings( 'arf_gmail_api_response_data', wp_json_encode($arflite_gmail_oauth_data), 'general_settings' );
                ...
                if( is_wp_error( $check_access_token ) ){
                    return wp_json_encode(array('success' => 'false', 'msg' => esc_html__('Something went wrong please check your configuration.','arforms-form-builder')));
                }
            ...
            try {
				...
				return wp_json_encode(array('success' => 'true', 'msg' => ''));
                ...
                return wp_json_encode(array('success' => 'false', 'msg' => $e));
            }
		} else {
			...
			$this->arforms_wpmail_sending = true;
			wp_mail($recipient, $subject, $message, $header, $attachments);
			$this->arforms_wpmail_sending = false;
		}
	...
	function arfliteentry_created( $entry_id, $form_id ) {
		...
		do_action( 'arflite_update_admin_email_notification_data_outside', $admin_email_notification_data, $entry_id, $form_id );
