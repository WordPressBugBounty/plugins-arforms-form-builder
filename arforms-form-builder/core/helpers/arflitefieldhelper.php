...
class arflitefieldhelper {
	...
	function arflite_get_default_field_options( $type, $field, $limit = false ) {
		...
		if ( $type == 'captcha' ) {
			...
			$field_options['options'] = wp_json_encode( array( 'Checkbox 1', 'Checkbox2' ) );
		} elseif ( $type == 'radio' ) {
			$field_options['options'] = wp_json_encode( array( 'Radio 1', 'Radio 2' ) );
		} elseif ( $type == 'select' ) {
			$field_options['options'] = wp_json_encode( array( '', 'Select 1' ) );
		}
		...
		if ( $frm_css['arfinputstyle'] == 'material' ) {
			...
			if ( $frm_css['arfinputstyle'] == 'material' ) {
				...
				<span class="arfeditorfieldopt_label arf_edit_in_place">
					<input type="text" class="arf_edit_in_place_input inplace_field"  onclick="arf_open_respective_style_option('arf_label')"  data-ajax="false" data-field-opt-change="true" data-field-opt-key='name' value="<?php echo stripslashes_deep( esc_attr( $field['name'] ) ); //phpcs:ignore ?>" data-field-id="<?php echo esc_attr( $field['id'] ); ?>" />
				</span>
