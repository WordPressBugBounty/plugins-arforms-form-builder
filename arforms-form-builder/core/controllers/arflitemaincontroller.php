...
class arflitemaincontroller {
	...
	function arflite_template_install(){
		global $arfdefaulttemplate;
        $arfdefaulttemplate = array(
            '1' => array('name' => addslashes(esc_html__('Contact us', 'arforms-form-builder')),'theme'=> addslashes(esc_html__('standard', 'arforms-form-builder'))),
            '2' => array('name' => addslashes(esc_html__('Subscription Form', 'arforms-form-builder')),'theme'=> addslashes(esc_html__('standard', 'arforms-form-builder'))),
            '3' => array('name' => addslashes(esc_html__('Feedback Form', 'arforms-form-builder')),'theme'=> addslashes(esc_html__('standard', 'arforms-form-builder'))),
        );
	}
	...
	function arflite_display_sale_popup_callback() {
		...
		if ( isset( $_GET['page'] ) && preg_match( '/ARForms*/', sanitize_text_field( $_GET['page'] ) ) ) {
			$sale_popup = '
				...
				<input type="hidden" name="arforms_validation_nonce" id="arforms_validation_nonce" value="'.esc_attr( wp_create_nonce( 'arf_wp_nonce' ) ).'" />
	...
	function arflite_disable_sale_popup() {
		...
		if (!current_user_can('manage_options')) {
            ...
            $response_data['message'] = esc_html__( 'Sorry, you are not allowed to install plugins on this site.','arforms-form-builder' );
            echo wp_json_encode($response_data);
            die;

		}
		if ( empty( $_POST['_wpnonce'] ) || (isset( $_POST['_wpnonce'] ) && '' != $_POST['_wpnonce'] && ! wp_verify_nonce( sanitize_text_field( $_POST['_wpnonce'] ), 'arf_wp_nonce' )) ) {
            $response_data['status'] = 'error';
            $response_data['message'] = esc_html__( 'Sorry, your request cannot be processed due to security reason.','arforms-form-builder' );
            echo wp_json_encode($response_data);
            die;
		}
	...
	function arflite_admin_js() {
		...
		global $arfliteversion, $pagenow, $arflitemaincontroller, $wp_version, $arformsmain;
		...
		if ( isset( $_GET ) && ( isset( $_GET['page'] ) && preg_match( '/ARForms*/', sanitize_text_field( $_GET['page'] ) ) ) || ( $pagenow == 'edit.php' && isset( $_GET ) && isset( $_GET['post_type'] ) && sanitize_text_field( $_GET['post_type'] ) == 'frm_display' ) ) {
			...
			wp_enqueue_script( 'arf_selectpicker', ARFLITEURL . '/js/arf_selectpicker.js', array( $jquery_handler ), $arfliteversion );
			...
			wp_enqueue_style( 'arf_selectpicker', ARFLITEURL . '/css/arf_selectpicker.css', array(), $arfliteversion );
	...
	function arflite_front_head( $ispost = '' ) {
		...
		if ( ! is_admin() ) {
			...
			wp_register_script( 'arf_selectpicker', ARFLITEURL . '/js/arf_selectpicker.js', array( 'jquery' ), $arfliteversion );

			wp_register_style( 'arf_selectpicker', ARFLITEURL . '/css/arf_selectpicker.css', array(), $arfliteversion );
	...
	function arflite_front_head_js() {
		global $post, $wpdb, $arfliteformcontroller, $arfliteversion, $arfliteform, $arflitemainhelper, $arfliterecordhelper, $arflitefieldhelper, $arflite_form_type_with_id, $ARFLiteMdlDb,$arflite_func_val, $arflite_jscss_version, $tbl_arf_forms, $arformsmain;
	...
	function arflite_load_assets_for_elementor_callback( $form_id = '', $action = '' ) {
		...
		wp_register_script( 'arf_selectpicker', ARFLITEURL . '/js/arf_selectpicker.js', array(), $arflite_jscss_version );
		wp_register_style( 'arf_selectpicker', ARFLITEURL . '/css/arf_selectpicker.css', array(), $arflite_jscss_version );
	...
	function arflite_set_js( $hook ) {
		...
		} elseif ( isset( $_REQUEST['page'] ) && $_REQUEST['page'] != '' && sanitize_text_field( $_REQUEST['page'] ) == 'ARForms' && ( isset( $_REQUEST['arfaction'] ) &&  (sanitize_text_field( $_REQUEST['arfaction'] ) == 'edit' || sanitize_text_field( $_REQUEST['arfaction'] ) == 'new' || sanitize_text_field( $_REQUEST['arfaction'] ) == 'duplicate' || sanitize_text_field( $_REQUEST['arfaction'] ) == 'update' )) ) {
		...
		$js_data = "__ARF_FIELD_TYPE_LABELS = '" . wp_json_encode( $field_type_label_array ) . "';";
		...
		$traslated_text = "
			...
			var __ARFLITE_CAPABILITY_ERROR = '" . addslashes( __( 'Sorry, you do not have permission to perform this action', 'arforms-form-builder' ) ) . "';

			var __ARFLITE_SECURITY_ERROR = '" . addslashes( __( 'Sorry, your request could not be processed due to security reason.', 'arforms-form-builder' ) ) . "';
	...
	function arflite_set_css() {
		...
		if ( isset( $_REQUEST['page'] ) && $_REQUEST['page'] != '' && sanitize_text_field( $_REQUEST['page'] ) == 'ARForms-settings' ) {
	...
	function arflite_upgrade_data() {
		...
		if ( version_compare( $arflitenewdbversion, '1.7.3', '<' ) ) {
	...
	function arflite_display_notice_for_rating() {
		...
		if ( '' != $display_notice && 'yes' == $display_notice && ( '' == $display_notice_never || 'true' != $display_notice_never ) ) {
			...
			wp_nonce_field( 'arflite_nonce', 'arflite_nonce', 'arflite_nonce', true );
	...
	function arflite_add_new_version_release_note() {
		...
		if ( isset( $_REQUEST['page'] ) && in_array( $_REQUEST['page'], $arf_slugs ) ) {
			...
			$popupData = '<div class="arf_modal_overlay arfactive">
				<input type="hidden" name="arforms_validation_nonce" id="arf_whatsnewpopup_nonce" value="'.esc_attr( wp_create_nonce( 'arf_wp_nonce' ) ).'" />
                                ...
                                <div class="arf_whatsnew_popup_inner_content">
                                    ...
                                    You can always refer our online documentation for all the features <a href="https://www.arformsplugin.com/documentation/1-getting-started-with-arforms/" target="_blank">here</a><br>
                                        <ul style="list-style-type: disc;">
											<li>Added new built-in option for Google reCaptcha</li>
											<li>Added new built-in option for Turnstile captcha</li>
											<li>Added new built-in option to collect payments through PayPal Standard</li>
											<li>Added new facility to disable IP logging</li>
											<li>Added new facility to read/unread form entries</li>
											<li>Added completely new three template forms</li>
											<li>Added new built-in feature to block empty form submission</li>
			...
			$popupData .= 'function arflite_hide_update_notice(){
				var _wpnonce = jQuery("#arf_whatsnewpopup_nonce").val();
                var ishide = 1;
                jQuery.ajax({
                type: "POST",
                url: "' . $arfliteajaxurl . '",
                data: "action=arflite_dont_show_upgrade_notice&is_hide=" + ishide + "&_wpnonce=" + _wpnonce,
	...
	function arflite_check_user_cap( $arflite_cap = '', $arflite_is_ajax_call = false ) {
		...
		{
			$arf_nonce = isset( $_REQUEST['_wpnonce'] ) ? sanitize_text_field( $_REQUEST['_wpnonce'] ) : '';

			if ( '' == $arf_nonce ) {
				$arf_nonce = isset( $_POST['_wpnonce'] ) ? sanitize_text_field( $_POST['_wpnonce'] ) : '';
			}
	
			$arf_verify_nonce_flag = wp_verify_nonce( $arf_nonce, 'arforms_wp_nonce' );
	
			if ( ! $arf_verify_nonce_flag ) {
