...
class arflitesettingcontroller {
	...
	function addons_page() {
		...
		$urltopost = 'https://www.arformsplugin.com/addonlist/addon_list_new_3.0.php';
		...
		if ( is_wp_error( $raw_response ) || $raw_response['response']['code'] != 200 ) {
			...
			$arforms_body_res = base64_decode( $raw_response['body'] );
			if ( ! empty( $arforms_body_res ) ) {
				$arforms_body_res = json_decode( $arforms_body_res, true );                        
				$arforms_addon_list_css = '';

				foreach ( $arforms_body_res as $arforms_body_key => $arforms_body_data_arr ) {
					foreach ( $arforms_body_data_arr as $arforms_body_data_key => $arforms_body_val ) {

						$arforms_horizontal_postion = isset($arforms_body_val['addon_icon_horizontal_position'])  ? $arforms_body_val['addon_icon_horizontal_position'] : 0;
						$addon_icon_vertical_position = isset($arforms_body_val['addon_icon_vertical_position'])  ? $arforms_body_val['addon_icon_vertical_position'] : 0;
						$addon_icon_slug = isset($arforms_body_val['arf_class'])  ? $arforms_body_val['arf_class'] : '';

						$arforms_addon_list_css .= '
							.addon_image.'.$addon_icon_slug.'{
								background-position: '.$arforms_horizontal_postion.' '.$addon_icon_vertical_position.';
							}';   
				...
				$arforms_addon_list_css .= '.addon_image{
					background: url("https://www.arformsplugin.com/addonlist/arf-addons-sprite.webp?ver='.current_time('timestamp').'") 0 0 no-repeat;
				}';

				echo "<style rel='text/stylesheet' id='arf_addon_style'>".$arforms_addon_list_css."</style>";
			}
	...
	function CheckpluginStatus( $mypluginsarray, $pluginname, $attr, $purchase_addon, $plugin_type, $install_url, $is_allowed_for_free = false ) {
		...
		if ( true == $is_allowed_for_free ) {
			...
			$myicon    = '<button class="addon_button" onClick="window.open(\'' . $actionurl . '\',\'_blank\')"><span>' . __( 'Upgade to Pro', 'arforms-form-builder' ) . '</span></button>';
		}
		return $myicon;
	}
	...
	function arflite_install_plugin() {
		...
		if ( empty( $_POST['_wpnonce'] ) || (isset( $_POST['_wpnonce'] ) && ! wp_verify_nonce( sanitize_text_field( $_POST['_wpnonce'] ), 'arf_wp_nonce' )) ) {
			echo esc_attr( 'security_error' );
			die;
		}
		...
		if(!str_contains($plugin_slug , 'arforms'))
       {
            echo wp_json_encode(array('error' => 'false', 'msg' => addslashes(esc_html__('You cannot install this Plugin as your are not using ARForms', 'arforms-form-builder'))));
            die;
       }
	...
	function arflite_activate_plugin() {
        ...
        if (empty($plugin_slug)) {
            wp_send_json_error(array(
                'slug' => '',
                'errorCode' => 'no_plugin_specified',
                'errorMessage' => addslashes(esc_html__('No plugin specified.', 'arforms-form-builder')),
            ));
        }

        $plugin = $plugin_slug;
        $plugin = plugin_basename(trim($plugin));
        $plugin_slug_check = explode("/", $plugin);
        $plugin_slug_check = $plugin_slug_check[0];

       if(!str_contains($plugin_slug_check , 'arforms'))
       	{
            echo wp_json_encode(array('error' => 'false', 'msg' => addslashes(esc_html__('You cannot install this Plugin as your are not using ARForms', 'arforms-form-builder'))));
            die;
       	}	
	...
	function arflite_deactivate_plugin() {
		...
		if( !empty($_POST['slug'] )){
            $plugin_slug_var = $_POST['slug'];
        }

        if (empty($plugin_slug_var)) {
            wp_send_json_error(array(
                'slug' => '',
                'errorCode' => 'no_plugin_specified',
                'errorMessage' => addslashes(esc_html__('No plugin specified.', 'arforms-form-builder')),
...
function arfliteobject2array( $object ) {
	return @json_decode( @wp_json_encode( $object ), 1 );
}
?>
