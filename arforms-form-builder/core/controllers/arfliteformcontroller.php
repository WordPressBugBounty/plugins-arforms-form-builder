...
class arfliteformcontroller {
	...
	function arflite_upload_image_from_admin() {
		...
		if ( $fn && isset( $_FILES['files']['name'] ) ) {
            ...
            if( in_array( strtolower($file_ext) , $arf_not_allowd_extention) ){
                ...
                echo '<p class="error_upload">'.esc_html__('Invalid file extension. Please select valid file','arforms-form-builder').'</p>';
                die;
            }
	...
	function arflite_process_bulk_form_actions( $arflite_errors ) {
		...
		if ( empty( $_GET['arflite_page_nonce'] ) || ( isset( $_GET['arflite_page_nonce'] ) && '' != $_GET['arflite_page_nonce'] && wp_verify_nonce( sanitize_text_field( $_GET['arflite_page_nonce'] ), 'arflite_page_nonce' ) ) ) {
			return;
		}
	...
	function arflite_filter_content( $content, $form, $entry = false ) {
		...
		if ( $entry && is_numeric( $entry ) ) {
			...
			if ( empty( $_POST['arflite_entry_nonce'] ) || ( isset( $_POST['arflite_entry_nonce'] ) && '' != $_POST['arflite_entry_nonce'] && ! wp_verify_nonce( sanitize_text_field( $_POST['arflite_entry_nonce'] ), 'arflite_entry_nonce' ) ) ) {
				return $content;
			}
	...
	function arflite_change_show_hide_column() {
		...
		if ( empty( $_POST['arflite_wp_nonce'] ) || ( isset( $_POST['arflite_wp_nonce'] ) && '' != $_POST['arflite_wp_nonce'] && ! wp_verify_nonce( sanitize_text_field( $_POST['arflite_wp_nonce'] ), 'arflite_wp_nonce' ) ) ) {
			echo '';
			die;
		}
	...
	function arfliteupdateformbulkoption() {
		...
		if ( empty( $_POST['_arforms_wpnonce'] ) || ( isset( $_POST['_arforms_wpnonce'] ) && '' != $_POST['_arforms_wpnonce'] && ! wp_verify_nonce( sanitize_text_field( $_POST['_arforms_wpnonce'] ), 'arflite_wp_nonce' ) ) ) {
			echo wp_json_encode(
				array(
					'error'       => true,
	...
	function arfliteformsavealloptions() {
		...
		if ( 'success' != $arflite_validate_request ) {
			...
			if ( ! empty( $arf_error_obj[1] ) && 'security_error' == $arf_error_obj[1] ) {
				...
				echo 'false^|^' . wp_json_encode( $arf_error_obj );
			}
			die;
		}
		$is_preview = false;
		...
		if ( count( $arflite_errors ) > 0 ) {
			echo 'false^|^' . wp_json_encode( $arflite_errors );
			die();
		}
		...
		if ( isset( $values['options'] ) ) {
			...
			$options['arf_field_order'] = isset( $values['arf_field_order'] ) ? sanitize_text_field( $values['arf_field_order'] ) : wp_json_encode( array() );

			$options['arf_field_resize_width'] = isset( $values['arf_field_resize_width'] ) ? sanitize_text_field( $values['arf_field_resize_width'] ) : wp_json_encode( array() );
		...
		foreach ( $values as $key => $value ) {
			if ( preg_match( '/(arf_field_data_)/', $key ) ) {
				...
				} elseif ( !empty( $field_otions_new['default_value'] ) && $field_otions_new['default_value'] != '' ) {
				...
				if ( isset( $field_otions_new['options'] ) && ! empty( $field_otions_new['options'] ) ) {
					if ( is_array( $field_otions_new['options'] ) ) {
						if ( $is_preview ) {
							...
							$field_options = wp_json_encode( $field_otions_new['options'] );
						}
				...
				if ( in_array( $field_id_new, $old_field_array ) ) {
					...
					if ( $field_otions_new['type'] == 'email' ) {
						if ( $field_otions_new['confirm_email'] == '1' ) {
							...
							if ( is_array( $options['arf_field_order'] ) || is_object( $options['arf_field_order'] ) ) {
								$confirm_field_order_arr = arflite_json_decode( wp_json_encode( $options['arf_field_order'] ), true );
							} else {
					...
					$insert_default_value = is_array( $default_value ) ? wp_json_encode( $default_value ) : $default_value;
					...
					if ( $is_preview ) {
						...
						$final_val = wp_json_encode( $new_val );
					}
		...
		if ( $options['arf_data_with_url'] != 0 ) {
			...
			if ( $options['arf_data_key_with_url'] != 0 ) {
				...
				$options['arf_field_key_name'] = wp_json_encode( $key_name_list );
			}
		}
		...
		if ( ! $is_preview ) {
			$options['arf_field_order'] = isset( $final_field_order ) ? wp_json_encode( $final_field_order ) : array();
		} else {
		...
		if ( $is_preview ) {
			...
			update_option( $option_name, addslashes( wp_json_encode( $arf_preview_form_data ) ) );
		...
		$return_json_data['arf_default_newarr']           = wp_json_encode( $temp_new_values );
		...
		$return_json_data_final = wp_json_encode( $return_json_data );
	...
	function arflite_get_form_hidden_field( $form, $fields, $values, $arflite_preview, $is_widget_or_modal, $arflite_data_uniq_id, $form_action, $loaded_field, $type, $is_close_link, $arf_current_token ) {
		...
		$arforms_script_data = array(
			'arforms_script_url' => ARFLITESCRIPTURL,
			'arforms_ajaxurl' => admin_url( 'admin-ajax.php' ),
			'arforms_blank_msg' => esc_html__( 'This field cannot be blank', 'arforms-form-builder' ) 
		);

		$hidden_fields .= '<input type="hidden" data-jqvalidate="false" id="arformslite_front_obj" value="'. wp_json_encode( $arforms_script_data ) . '" />';
		...
		$hidden_fields .= '<input type="hidden" data-jqvalidate="false" name="arflite_nonce" id="arflite_nonce" value="'.wp_create_nonce('arflite_nonce').'" />';
	...
	function arflite_get_all_field_html( $form, $values, $arflite_data_uniq_id, $fields, $arflite_preview, $arflite_errors, $inputStyle, $arf_arr_preset_data = array() ) {
		...
		foreach ( $arf_fields as $field ) {
			...
			if ( is_array( $field ) || is_object( $field ) ) {
				...
				if ( $field['name'] != '' ) {
					$aria_label = isset($field['aria_label']) ? $field['aria_label'] : '';
					$arf_label_for_attribute = 'for="field_' . $field['field_key'] . '_' . $arflite_data_uniq_id . '" aria-label= "'. $aria_label.'"';
				...
				if ( isset( $field['field_width'] ) && $field['field_width'] != '' ) {
					$arflite_form_all_footer_css .= '#arffrm_'.$form_data->id.'_container .arf_fieldset .arfformfield#arf_field_' . $field['id'] . '_' . $arflite_data_uniq_id . '_container .controls{';
					...
					if ( isset( $field['enable_arf_prefix'] ) && $field['enable_arf_prefix'] != 1 && isset( $field['enable_arf_suffix'] ) && $field['enable_arf_suffix'] != 1 ) {
						$arflite_form_all_footer_css     .= '#arffrm_'.$form_data->id.'_container .arf_fieldset .arfformfield#arf_field_' . $field['id'] . '_' . $arflite_data_uniq_id . '_container .controls input{';
							$arflite_form_all_footer_css .= 'width:' . $field['field_width'] . 'px;';
						$arflite_form_all_footer_css     .= '}';
					}

					if ( 'textarea' == $field['type'] ) {
						$arflite_form_all_footer_css     .= '#arffrm_'.$form_data->id.'_container .arf_fieldset .arfformfield#arf_field_' . $field['id'] . '_' . $arflite_data_uniq_id . '_container .controls textarea{';
				...
				$class_selector = (isset($field['class_selector']) && $field['class_selector'] != "") ? sanitize_text_field($field['class_selector']) : "";
				switch ( $field['type'] ) {
					case 'text':
						$return_string .= '<div id="arf_field_' . $field['id'] . '_' . $arflite_data_uniq_id . '_container" class="  '. $class_selector .'   arffomfield control-group arfmainformfield ' . $required_class . ' ' . $class_position . '' . $error_class . ' arf_field_' . $field['id'] . '" data-field-type="' . $field['type'] . '" data-parent_field="' . $parent_field_id . '" >';
						if ( $inputStyle != 'material' ) {
						...
						if ( apply_filters( 'arflite_check_for_draw_outside', false, $field ) ) {
							...
							if ( isset( $field['field_width'] ) && $field['field_width'] != '' and $field['enable_arf_prefix'] != 1 and $field['enable_arf_suffix'] != 1 ) {
					...
					case 'textarea':
						$return_string .= '<div id="arf_field_' . $field['id'] . '_' . $arflite_data_uniq_id . '_container" class="  '. $class_selector .'  arfformfield control-group arfmainformfield ' . $required_class . ' ' . $class_position . '' . $error_class . ' arf_field_' . $field['id'] . '"  data-field-type="' . $field['type'] . '" data-parent_field="' . $parent_field_id . '">';
						if ( $inputStyle != 'material' ) {
							$return_string .= $arf_main_label;
						}
						$return_string .= '<div class="controls' . $field_tooltip_class . '" ' . $field_tooltip . ' >';
					...
					case 'checkbox':
						if ( $inputStyle == 'material' ) {
							...
							$return_string  .= '<div id="arf_field_' . $field['id'] . '_' . $arflite_data_uniq_id . '_container" class="  '. $class_selector .'  arfformfield control-group arfmainformfield ' . $alignment_class . ' ' . $required_class . ' ' . $error_class . ' ' . $class_position . ' arf_field_' . $field['id'] . '"  data-field-type="' . $field['type'] . '" data-parent_field="' . $parent_field_id . '">';
							$return_string  .= $arf_main_label;
								...
								if ( apply_filters( 'arflite_check_for_draw_outside', false, $field ) ) {
									...
									foreach ( $field['options'] as $opt_key => $opt ) {
										...
										if ( ! isset( $atts ) || ! isset( $atts['label'] ) || $atts['label'] ) {
											...
											$return_string .= '</span>';
											$return_string .= '</div>';
											$return_string .= '<label data-type="checkbox" for="field_' . $field['id'] . '-' . $opt_key . '-' . $arflite_data_uniq_id . '" aria-label="' . esc_attr($opt) . '">';
											if ( $field['use_image'] == 1 && $label_image != '' ) {
												$temp_check = '';
							...
							$alignment_class = ( isset( $field['align'] ) && $field['align'] == 'block' ) ? ' arf_vertical_radio' : ' arf_horizontal_radio';
							$return_string  .= '<div id="arf_field_' . $field['id'] . '_' . $arflite_data_uniq_id . '_container" class=" '. $class_selector .'  arfformfield control-group arfmainformfield ' . $alignment_class . ' ' . $required_class . '  ' . $class_position . ' ' . $error_class . ' arf_field_' . $field['id'] . '"  data-field-type="' . $field['type'] . '" data-parent_field="' . $parent_field_id . '">';
							$return_string  .= $arf_main_label;
					...
					case 'radio':
								...
								if ( apply_filters( 'arflite_check_for_draw_outside', false, $field ) ) {
									...
									foreach ( $field['options'] as $opt_key => $opt ) {
										...
										if ( ! isset( $atts ) || ! isset( $atts['label'] ) || $atts['label'] ) {
											...
											$return_string .= '<label data-type="radio" for="field_' . $field['id'] . '-' . $opt_key . '-' . $arflite_data_uniq_id . '" class="' . $arf_radion_image_class . '" aria-label="'. esc_attr($opt).'">';
											if ( $field['use_image'] == 1 && $label_image != '' ) {
								...
								if ( apply_filters( 'arflite_check_for_draw_outside', false, $field ) ) {
									...
									foreach ( $field['options'] as $opt_key => $opt ) {
										...
										if ( ! isset( $atts ) || ! isset( $atts['label'] ) || $atts['label'] ) {
											...
											$return_string .= '<label data-type="radio" for="field_' . $field['id'] . '-' . $opt_key . '-' . $arflite_data_uniq_id . '" class="' . $arf_radion_image_class . '" aira-label="'. esc_attr( $opt ).'">';
						...
						break;
					case 'select':
							$return_string .= '<div id="arf_field_' . $field['id'] . '_' . $arflite_data_uniq_id . '_container" class="  '. $class_selector .'  arfformfield input-field control-group arfmainformfield ' . $required_class . ' ' . $class_position . ' ' . $error_class . ' arf_field_' . $field['id'] . '"  data-field-type="' . $field['type'] . '" data-parent_field="' . $parent_field_id . '">';
							if ( $inputStyle != 'material' ) {
								$return_string .= $arf_main_label;
							}
						...
						if ( apply_filters( 'arflite_check_for_draw_outside', false, $field ) ) {
							...
							if ( ! empty( $field['options'] ) ) {
								$count_i = 0;
								foreach ( $field['options'] as $opt_key => $opt ) {
										$first_label = ( isset( $field['separate_value'] ) && $field['separate_value'] == 1 && isset($opt['label']) && $opt['label'] != "" ) ? $opt['label'] : $field['placeholdertext'];
									$field_val = apply_filters( 'arflitedisplaysavedfieldvalue', $opt, $opt_key, $field );
						...
						break;
					case 'number':
						$return_string .= '<div id="arf_field_' . $field['id'] . '_' . $arflite_data_uniq_id . '_container" class="  '. $class_selector .'  arfformfield control-group arfmainformfield ' . $material_input_cls . ' ' . $required_class . ' ' . $class_position . ' ' . $error_class . ' arf_field_' . $field['id'] . '"  data-field-type="' . $field['type'] . '" data-parent_field="' . $parent_field_id . '">';
					...
					case 'tel':
						...
						if ( isset( $field['phonetype'] ) && $field['phonetype'] == 1 ) {
							...
							if ( isset( $phtypes ) && count( $phtypes ) > 0 ) {
								$return_string .= "<input type='hidden' data-jqvalidate='false' id='field_" . esc_attr( $field['field_key'] ) . "_country_list' value='" . esc_attr( wp_json_encode( $phtypes ) ) . "' />";
							}
					...
					case 'date':
						...
						if ( apply_filters( 'arflite_check_for_draw_outside', false, $field ) ) {
							...
							$data_off_days = '';
							if ( ! empty( $off_days ) ) {
								$data_off_days = "data-off-days='" . wp_json_encode( $off_days ) . "'";
							}
					...
					case 'time':
						...
						if ( apply_filters( 'arflite_check_for_draw_outside', false, $field ) ) {
							...
							$return_string .= ' data-field-options="' . htmlspecialchars( wp_json_encode( $time_field_options ) ) . '"';
					...
					case 'hidden':
						...
						break;
					case 'html':
						$return_string .= '<div id="arf_field_' . $field['id'] . '_' . $arflite_data_uniq_id . '_container" class="   '. $class_selector .'  arfformfield control-group arfmainformfield ' . $required_class . ' ' . $class_position . ' ' . $error_class . ' arf_field_' . $field['id'] . '" data-field-type="html" data-parent_field="' . $parent_field_id . '" >';
	...
	function arfliteArraytoObj( $array ) {
		if ( is_array( $array ) ) {
			$array = json_decode( wp_json_encode( $array ) );
		}
		return $array;
	}
	...
	function arflite_csv_form_function() {
		...
		$arflite_validate_request = $arflitemaincontroller->arflite_check_user_cap( 'arfviewentries', true );
			...
			foreach ( $unsaved_fields as $key => $value ) {
				...
				foreach ( $opts as $k => $val ) {
					if ( $k == 'key' ) {
						...
						$temp_fields[ $key ][ $k ] = ( $val != '' ) ? wp_json_encode( $val ) : '';
					} else {
			...
			if ( is_array( $fval ) ) {
				foreach ( $fval as $fki => $fvali ) {
					if ( 'name' == $fki || 'description' == $fki || 'arf_tooltip' == $fki || 'tooltip_text' == $fki ) {
						...
						$arf_fields[ $fk ][ $fki ] = wp_json_encode( $fvali_arr );
	...
	function arflite_load_form_css( $form_id, $inputStyle ) {
		...
		if($is_divibuilder)
		{
			$arf_selectpiker = ARFLITEURL . "/css/arf_selectpicker.css?ver=1.6.0";
			$arflite_gutenberg_materialize_css= ARFLITEURL."/css/arflite_front.css?ver=$arflite_jscss_version";
			$return_link .= "<link rel='stylesheet' type='text/css' id='{$stylesheet_handler}-fallback-css' href='{$fid}' />";
			$return_link .= "<link rel='stylesheet' type='text/css' id='{$stylesheet_handler}-fallback-css' href='{$arflite_gutenberg_materialize_css}' />";
	...
	function arflite_remove_preview_data() {
		...
		if ( $opt_id != '' && preg_match( '/^arflite_previewtabledata_/', $opt_id ) ) {
