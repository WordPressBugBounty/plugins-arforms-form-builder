...
class arfliterecordcontroller {
	...
	function __construct() {
		...
		add_action( 'wp_ajax_arf_read_unread_flag', array( $this, 'arflite_read_unread_function' ) );
	...
	function arfliteprocess_entry( $arflite_errors = '' ) {
		...
		global $arflite_db_record, $arfliteform, $arflitecreatedentry, $arfliteform_params, $arfliterecordcontroller,$tbl_arf_fields;
		...
		if ( empty( $arflite_errors ) && @count( $arf_errors ) == 0 ) {
			...
			if ( $params['action'] == 'create' ) {
				...
				$item_meta = isset($_POST['item_meta']) ? array_map( 'sanitize_text_field', $_POST['item_meta'] ): array();

				$nonEmptyItemValues = array_filter($item_meta, function($value) {
					return !empty($value);
				});

				$countNonEmptyItemValues = count($nonEmptyItemValues);

				
				$arf_fields_data_count = $wpdb->get_var( $wpdb->prepare( "SELECT COUNT(*) FROM {$tbl_arf_fields} WHERE form_id = %d AND type != %s", $form_id, 'html' ) );

				if($arf_fields_data_count > 0 )
				{
					if($countNonEmptyItemValues == 0)
					{
						$empty_msg = $arformsmain->arforms_get_settings('empty_msg','general_settings');
						$empty_msg = !empty( $empty_msg ) ? $empty_msg : addcslashes(esc_html('Sorry, you cannot submit an empty form..','arforms-form-builder'));
						$message               = '<div class="arf_form arflite_main_div_{arf_form_id} arf_error_wrapper" id="arffrm_{arf_form_id}_container"><div 		class="frm_error_style" id="arf_message_error"><div class="msg-detail"><div class="arf_res_front_msg_desc">' .$empty_msg. '</div></div></div></div>';
						
						if( !empty( $_POST['using_ajax'] ) && 'yes' == $_POST['using_ajax'] ){
							$return['conf_method'] = 'spamerror';
							$return['message']     = $message;
							$return                = apply_filters( 'arflite_reset_built_in_captcha', $return, $_POST ); //phpcs:ignore
							echo wp_json_encode( $return );
							...
							$empty_msg = $arformsmain->arforms_get_settings('empty_msg','general_settings');
							$empty_msg = !empty( $empty_msg ) ? $empty_msg : addcslashes(esc_html('Sorry, you cannot submit an empty form..','arforms-form-builder'));
							do_shortcode( '[ARForms id=' . intval( $_POST['form_id'] ) . " arfsubmiterrormsg='" . $empty_msg . "' ]" ); //phpcs:ignore
							return;
						}
					}
				}
	...
	function arflite_footer_js( $arflite_preview = false, $is_print = false, $force_load = false, $is_return = false ) {
		...
		if ( $load_data && !$arformsmain->arforms_is_pro_active()) {
			...
			if ( $arflite_preview == true ) {
				$script_data .= '<input type="hidden" class="arflite_preview_form" value="' . esc_attr( base64_encode( wp_json_encode( $arflite_forms_loaded[0] ) ) ) . '" />';
			}
			...
			if ( isset( $arflite_form_all_footer_js ) ) {
				...
				if ( $arflite_preview ) {
					if( false == $is_return ){	
						...
						wp_print_scripts( 'arforms-control' );
					} else {
						wp_enqueue_script( 'arformslite-js' );
						wp_enqueue_script( 'arforms-control' );
					}
				}
			}
	...
	function arflitechangebulkentries() {
		...
		global $arflitemainhelper, $wpdb, $ARFLiteMdlDb,$arformsmain,$arflite_db_record,$arfsettings,$tbl_arf_entries, $tbl_arf_forms;

		if( ! current_user_can( 'arfviewentries' ) ){
			echo wp_json_encode(
                array(
                    'error' => true,
                    'message' => addslashes( esc_html__( 'Sorry, you do not have permission to perform this action', 'arforms-form-builder' ) )
                )
            );
            die;
        }

        if ( empty( $_REQUEST['_wpnonce_arflite'] ) || (isset( $_REQUEST['_wpnonce_arflite'] ) && '' != $_REQUEST['_wpnonce_arflite'] && ! wp_verify_nonce( sanitize_text_field( $_REQUEST['_wpnonce_arflite'] ), 'arflite_wp_nonce' )) ) {
            
            echo wp_json_encode(
                array(
                    'error' => true,
                    'message' => addslashes( esc_html__( 'Sorry, your request cannot be processed due to security reason.', 'arforms-form-builder' ) )
                )
            );
            die;
        }
			...
			if ( ! current_user_can( 'arfdeleteentries' ) ) {
				...
				if ( is_array( $items ) ) {
					...
					if ( $del_res ) {
						...
						if( $arformsmain->arforms_is_pro_active() ){
							$unread_entries_value = $wpdb->get_var( $wpdb->prepare( "SELECT COUNT(*) FROM {$tbl_arf_entries} ent LEFT JOIN {$tbl_arf_forms} frm ON ent.form_id = frm.id WHERE ent.status = %d AND frm.is_template = %d AND frm.arf_is_lite_form = %d AND frm.status = %s", 0, 0, 0, 'published'));
						} else {
							$unread_entries_value = $wpdb->get_var( $wpdb->prepare( "SELECT COUNT(*) FROM $tbl_arf_entries  ent LEFT JOIN {$tbl_arf_forms} frm ON ent.form_id = frm.id WHERE ent.status = %d",0 ) );
						}
						...
						echo wp_json_encode(
							array(
								...
								'unread_entries'=>$unread_entries_value
							)
						);
					}
				}
		...
		elseif ( $bulk_action == 'mark_read')
		{
			...
			$arf_update_entry=array(
				'status' => 1
			);

			$update_query = $wpdb->prepare(
				"UPDATE $tbl_arf_entries SET status = %d WHERE id IN ($ids)",
				$arf_update_entry['status']
			);
			...
			$message = __( 'Entries Read successfully.', 'arforms-form-builder' );
			echo wp_json_encode(
				array(
					'error'     => false,
					'message'   => $message,
		...
		elseif($bulk_action == 'mark_unread')
		{
			...
			$message = __( 'Entries Unread successfully.', 'arforms-form-builder' );
			echo wp_json_encode(
				array(
					'error'     => false,
					'message'   => $message,
	...
	function arflite_retrieve_form_entry_data() {
		...
		$response = array();
		if ( empty( $_POST['_wpnonce_arforms'] ) || (isset( $_POST['_wpnonce_arforms'] ) && '' != $_POST['_wpnonce_arforms'] && ! wp_verify_nonce( sanitize_text_field( $_POST['_wpnonce_arforms'] ), 'arforms_wp_nonce' )) ) {
		...
		if ( count( $items ) > 0 ) {
			...
			foreach ( $items as $key => $item ) {
				...
				foreach ( $form_cols as $col ) {
					...
					if ( $col->type == 'checkbox' || $col->type == 'radio' || $col->type == 'select' ) {
						if ( isset( $col->field_options['separate_value'] ) && $col->field_options['separate_value'] == '1' ) {
							...
							$arf_edit_select_array[] = array( $col->id => wp_json_encode( $option_separate_value, JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_QUOT | JSON_HEX_AMP ) );
						} else {
							$option_value = '';
							$option_value = array();
							if ( is_array( $col->options ) ) {
							...
							$arf_edit_select_array[] = array( $col->id => wp_json_encode( $option_value, JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_QUOT | JSON_HEX_AMP ) );
						}
					}
				...
				$arf_entry_status = $wpdb->get_var( $wpdb->prepare( "SELECT status FROM {$tbl_arf_entries} WHERE id = %d", $item->id ) );

				$arf_entry_status_cls = ( 1 == $arf_entry_status ) ? 'read' : 'unread';

				$entry_status = "<input type='hidden' id='status_{$item->id}' value='{$arf_entry_status}' class='arf_status_{$arf_entry_status_cls}' />";

				$data[ $ai ][ $ni + 7 ]  = "<div class='arf-row-actions'>{$view_entry_btn}{$delete_entry_btn} {$delete_entry_overlay} {$entry_status}</div>";
				$data[ $ai ][ $ni + 7 ] .= "<input type='hidden' id='arf_edit_select_array_{$item->id}' value='" . wp_json_encode( $arf_edit_select_array ) . "' />";
				$arflite_PDF_button      = '';
				$action_no               = $ni + 7;
				$ai++;
			}
	...
	function arflite_get_entries_list_edit( $id = '', $arffieldorder = array(), $arfinnerfieldorder = array() ) {
		...
		$json_data = wp_json_encode( $as_edit_entry_value );
	...
	function arflite_ajax_check_recaptcha() {
		...
		echo wp_json_encode( $arflite_errors );
		die();
	}
	...
	function arflite_read_unread_function()
	{
		global $tbl_arf_entries ,$wpdb,$arformsmain, $tbl_arf_forms;
		if ( empty( $_POST['_wpnonce_arflite'] ) || (isset( $_POST['_wpnonce_arflite'] ) && '' != $_POST['_wpnonce_arflite'] && ! wp_verify_nonce( sanitize_text_field( $_POST['_wpnonce_arflite'] ), 'arforms_wp_nonce' )) ) {
            echo wp_json_encode(
				array(
					'error'   => true,
					'message' =>
		...
		$wpdb->update( 
			$tbl_arf_entries,                // Table name
			$arf_update_entry,               // Data to update (associative array)
			$where,                          // WHERE clause (associative array to specify the condition)
			array('%d'),                     // Optional. An array of formats for `$data`.
			array('%d')                      // Optional. An array of formats for `$where`.
		);
	...
	function arflite_delete_single_entry_function() {
		global $arflite_db_record , $tbl_arf_entries,$wpdb, $tbl_arf_forms, $arformsmain;

		if ( empty( $_POST['_wpnonce'] ) || (isset( $_POST['_wpnonce'] ) && '' != $_POST['_wpnonce'] && ! wp_verify_nonce( sanitize_text_field( $_POST['_wpnonce'] ), 'arforms_wp_nonce' )) ) {
            echo wp_json_encode(
				array(
					'error'   => true,
					'message' =>
